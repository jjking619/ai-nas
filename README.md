...
> 先说结论：最小安装前置条件只有两项：`/home/pi/nas_share`（根挂载目录）和 `/home/pi/nas_share/工作文档`（知识库入口）。不要手动创建 `downloads`、`家庭相册`、`手机相册`、`旅行`、`备份`、`tools` 等子目录；它们会在脚本/容器运行时按需自动生成。
...
```bash
cd /home/pi/NAS-Demo
bash install.sh reset     # 等价于旧版 reset-config.sh + docker restart openclaw
```
